...
import { panZoom } from "./Previewer.utils"
...
export function Previewer(props: IPreviewerProps) {
  const { children, initialScale = 0.5 } = props
  ...
  const applyPanZoom = React.useCallback(
    ({ reset = false } = {}) => {
      ...
      if (overlay && container) {
        ...
        const position = {
          x: -overlay.clientHeight + 16,
          y: -overlay.clientWidth + 16,
  ...
  return (
    <div className={cx(styles["previewer"])}>
      ...
      <div className={cx(styles["previewer-controls"])}>
        <button
          role="button"
          title="Switch to select mode"
          aria-label="Switch to select mode"
          className={cx(mode.is.select && styles["active"])}
        ...
        <button
          ...
          className={cx(mode.is.pan && styles["active"])}
          onClick={switchModeTo.pan}
        >
          <PanSvg />
        </button>
        <button role="button" title="Reset" aria-label="Reset" onClick={handleReset}>
          <ResetSvg />
        </button>
      </div>
    </div>
