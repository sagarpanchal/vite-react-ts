...
export function MovableWindow(props: MovableWindowProps) {
  ...
  return (
    <div tabIndex={0} ref={windowRef} className={cx(styles["previewer-window"])}>
      <div tabIndex={0} ref={titlebarRef} className={cx(styles["previewer-window-titlebar"])}>
        {title}
      </div>
      <div tabIndex={0} className={cx(styles["previewer-window-body"])}>
        {children}
      </div>
    </div>
  )
