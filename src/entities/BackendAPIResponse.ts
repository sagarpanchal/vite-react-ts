...
export interface IBackendAPIResponseParams {
...
export class BackendAPIResponse<T, O> {
  ...
  constructor(params: IBackendAPIResponseParams = {}) {
    this.#error = params?.error
    this.#response = params?.response as AxiosResponse<T>
    this.#result = params?.result as O
  }

  set error(error: IBackendAPIResponseParams["error"]) {
  ...
  set response(response: IBackendAPIResponseParams["response"]) {
